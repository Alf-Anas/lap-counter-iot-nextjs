...
export default function LapCard() {
    ...
    function getLapMax() {
        const redCar = filterDataByLap(
            allItem.filter((item) => item.car === CAR_NAME.RED)
        );
        const greenCar = filterDataByLap(
            allItem.filter((item) => item.car === CAR_NAME.GREEN)
        );
        const blueCar = filterDataByLap(
            allItem.filter((item) => item.car === CAR_NAME.BLUE)
        );
        const noneCar = filterDataByLap(
            allItem.filter(
                (item) =>
